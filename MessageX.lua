...
local function CreateItem(tbl,width,height,Flags,Buttons,Title,ExecDelay)
  ...
  local X3,X4 = X2-3,X2-4
  local buffer=far_CreateUserControl(X4,tbllen)
  ...
  for y=1,tbllen do
    if not leftAlign then
      local half=math_floor((X4-tbl[y]:len())/2)
      if half>0 then
        tbl[y]=string_rep(" ",half)..tbl[y]
        for x=X4,1,-1 do if ct[y][x] then ct[y][x+half]=ct[y][x] ct[y][x]=nil end end
      end
    end
    for x=1,X4 do
  ...
  return tButtons,butdef,X2,Y2,{F.DI_USERCONTROL,2,1,X3,tbllen,buffer,0,0,F.DIF_NOFOCUS,""}
end
...
local function MessageX(Msg,Title,Buttons,Flags,HelpTopic,Guid,ExecDelay)
  ...
  if w then width,height = w.Right-w.Left+1,w.Bottom-w.Top+1 end
