...
local Compare = function(p1,p2)
  local res,sz1,sz2,fa1,fa2,hs1,hs2 = 0
  if ts[2] then
  ...
  if ts[6]~=2 then
    sz1,sz2 = tonumber(p1.FileSize),tonumber(p2.FileSize)
    res=sz1-sz2
    if res~=0 then return res end
  end
  if ts[7]~=2 then
    if not sz1 then sz1,sz2 = tonumber(p1.FileSize),tonumber(p2.FileSize) end
    if sz1~=0 then
      hs1,hs2 = GetHash(p1.FileName),GetHash(p2.FileName)
  ...
  if ts[8]~=2 then
    fa1,fa2 = tonumber(p1.FileAttributes),tonumber(p2.FileAttributes)
    res=fa1-fa2
  end
  return res
end
...
action=function()
  if far.Dialog(uGuid,-1,-1,57,#Items,nil,Items,nil,DlgProc)==#Items-1 then
